...
#    ofile.write(pydoc.render_doc(sys.modules[__name__]),file=sys.stderr);
#    sys.exit();
...
#    argvi=argvi+1;
#    if len(ms)!=2:
#      ofile.write('Error: incorrect parameter for -e.',file=sys.stderr);
#      sys.exit(); 
#    try:
#      mu=float(ms[0]);
#      sigma=float(ms[1]);
...
#      ofile.write('Error: incorrect parameter for -e.',file=sys.stderr);
#      sys.exit(); 
#    ofile.write('Mean and variance for lognormal distribution: '+str(mu)+','+str(sigma),file=sys.stderr);
#  elif sys.argv[argvi]=="--geometric":
#    distype="geometric";
#    try:
#      mu=float(sys.argv[argvi+1]);
#      if mu<0 or mu>1:
#        ofile.write('Error: the parameter for geometric distribution must be between 0 and 1.',file=sys.stderr);
#        sys.exit(); 
#    except ValueError:
#      ofile.write('Error: incorrect parameter for -e.',file=sys.stderr);
#      sys.exit(); 
#    ofile.write('Mean for geometric distribution: '+str(mu),file=sys.stderr);
#    argvi=argvi+1;
#  else:
...
class GenEXPLVPlugin:
 def input(self, inputfile):
     allfile.append(inputfile)
 def run(self):
     pass
 def output(self, outputfile):
  ...
  ofile.write('#ID\tLength\tDir\tExons\tPosition\tGroupID\tNIsoformInGroup');
  if assignexplv==True:
    ofile.write('\tExplv\n');
  else:
    ofile.write("\n");
    ...
    if chrname!=prevchr or jstart-prevrange[1]>0:
      if len(prevchr)!=0:
        ...
        for item in currentgene:
          ofile.write(item[0]+"\t"+str(groupid)+"\t"+str(len(currentgene)));
          if assignexplv==True:
      ...
      ofile.write('Warning: the range is not sorted at line '+str(nline),file=sys.stderr);
    else:
      if jend>prevrange[1]:
        prevrange[1]=jend;
